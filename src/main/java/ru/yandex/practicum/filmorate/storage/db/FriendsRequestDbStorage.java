...
public class FriendsRequestDbStorage extends BaseDbStorage<FriendRequest> {
    ...
    private static final String ADD_FRIEND_QUERY = "INSERT INTO friend_request (to_user_id, from_user_id, status_id)" +
            "VALUES (?, ?, 1)";
    private static final String FIND_FRIENDS_BY_ID_QUERY = "SELECT *\n" +
            ...
            "    WHERE status_id = 1\n" +
            ");";
    private static final String UPDATE_QUERY = "UPDATE friend_request SET status = ? WHERE to_user_id = ? AND from_user_id = ?;";
    private static final String DELETE_QUERY = "DELETE FROM friend_request WHERE to_user_id = ? AND from_user_id = ?";
    private static final String FIND_BY_FRIENDS_ID_QUERY = "SELECT * FROM friend_request WHERE to_user_id = ? AND from_user_id = ?;";
